...
AddEventHandler('mythic_hospital:client:SendToBed', function(id, data)
    ...
    Citizen.CreateThread(function ()
        ...
        exports['mythic_notify']:SendAlert('inform', 'Du blir vårdad.')
...
AddEventHandler('mythic_hospital:client:FinishServices', function()
    ...
    exports['mythic_notify']:SendAlert('inform', 'Du har blivit fakturerad.')
    LeaveBed()
end)
        ...
        if distance < 10 then
            ...
            if not IsPedInAnyVehicle(PlayerPedId(), true) then
                if distance < 3 then
                    ...
                    ESX.Game.Utils.DrawText3D(vector3(hospitalCheckin.x, hospitalCheckin.y, hospitalCheckin.z + 0.5), '[~p~E~s~] För att få vård (~p~500kr~s~)', 0.4)
					if IsControlJustReleased(0, 54) then
                        if (GetEntityHealth(PlayerPedId()) < 200) or (IsInjuredOrBleeding()) then
                            ...
                            exports['mythic_notify']:SendAlert('error', 'Du behöver ingen vård')
                        end
                    end
                end
            end
