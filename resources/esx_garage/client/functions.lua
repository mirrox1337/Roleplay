    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        for key, vehicleData in ipairs(fetchedVehicles) do
            ...
            table.insert(menuElements, {
                ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " med plåten - " .. vehicleData["plate"],
                ["vehicle"] = vehicleData,
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Du har inga fordon i Garage " .. currentGarage
            })
        ...
        ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_garage_menu", {
            ...
            if currentVehicle then
                ...
                SpawnVehicle(currentVehicle["props"], menuData["current"]["fuel"])
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " med plåt - " .. vehicleData["plate"] .. " - " .. dstCheck .. " meter iväg.",
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    local menuElements = {
        ...
        -- },
        {
            ["label"] = "Sätt " .. (GetIsVehicleEngineRunning(vehicleEntity) and "AV" or "PÅ") .. " motorn.",
            ["action"] = "change_engine_state"
        },
        {
            ["label"] = "Sätt " .. (DoesBlipExist(cachedData["blips"][vehicleEntity]) and "AV" or "PÅ") .. " GPS-tracker.",
            ["action"] = "change_gps_state"
        },
        -- {
        --     ["label"] = "Kontrollera dörrarna.",
        --     ["action"] = "change_door_state"
        -- },
    }

    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Handlingar med " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Du är för långt borta från fordonet för att kontrollera det.") end
    ...
    elseif action == "change_gps_state" then
        if DoesBlipExist(cachedData["blips"][vehicleEntity]) then
            ...
            AddTextComponentString("Personligt fordon - " .. GetVehicleNumberPlateText(vehicleEntity))
...
SpawnLocalVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Var snäll att flytta på fordonet som står i vägen..")
...
SpawnVehicle = function(vehicleProps, fuel)
	local spawnpoint = Config.Garages[cachedData["currentGarage"]]["positions"]["vehicle"]
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Var snäll att flytta fordonet som står i vägen...")
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Detta fordon är på gatorna, du kan inte ta ut två av samma fordon.")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("Du tog ut ditt fordon.")

        TriggerEvent("setFuel", yourVehicle, fuel)
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("Du parkerade ditt fordon i garaget.")
			else
				ESX.ShowNotification("Du äger inte detta fordon.")
			end

		end, vehicleProps, cachedData["currentGarage"], GetVehicleFuelLevel(vehicle))
	end
end
...
WaitForModel = function(model)
    ...
    if not IsModelValid(model) then
        return ESX.ShowNotification("Den här modellen existerar inte..")
    end
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("Laddar modell.. " .. GetLabelText(GetDisplayNameFromVehicleModel(model)) .. "...", 255, 255, 255, 150)
	end
end