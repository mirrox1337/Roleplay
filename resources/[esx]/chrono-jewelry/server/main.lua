...
AddEventHandler("chrono-jewlery:retrieveItem", function()
    ...
    if luck >= 0 and luck <= 50 then
        ...
        player.addInventoryItem("safetyhammer", randomItem)
...
AddEventHandler('chrono-jewelry:rob', function(robb)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	local xPlayers = ESX.GetPlayers()
	local ItemQuantity = xPlayer.getInventoryItem('safetyhammer').count
	...
	if Stores[robb] then
		...
		if hasItem == true and rob == false then
			...
			local xPlayer = ESX.GetPlayerFromId(source)
			xPlayer.removeInventoryItem('safetyhammer', 1)
...
ESX.RegisterServerCallback('chrono-jewelry:getItemAmount', function(source, cb, item)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('chrono-jewelry:itemState', function(source, cb)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	local getState = xPlayer.getInventoryItem('safetyhammer').count
	if getState >= 1 then
		itemState = 1
	else
		itemState = 0
