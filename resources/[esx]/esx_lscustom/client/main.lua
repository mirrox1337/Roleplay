...
AddEventHandler('esx_lscustom:installMod', function()
	...
	curvCoords = GetEntityCoords(vehicle)
			...
			if (PlayerData.job and PlayerData.job.name == 'bennys') or not Config.IsBennysJobOnly then
				for k,v in pairs(Config.Zones) do
					if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x and not lsMenuIsShowed then
						...
						isInLSMarker  = false
						if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < v.Size.x+10 and not lsMenuIsShowed then
							drawText = true
							Draw3DText(v.Pos.x,v.Pos.y,v.Pos.z+1.35, "~w~Fordonsplats")
						end
					end
				end
			end
...
Citizen.CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(playerPed, false) and installedMod == true then
			local coords = GetEntityCoords(PlayerPedId())
			local currentZone, zone, lastZone
...
function Draw3DText(x, y, z, text)
    ...
    if onScreen and drawText == true then
