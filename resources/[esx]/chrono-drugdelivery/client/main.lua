...
--Citizen.CreateThread(function()
		...
		--AddTextComponentString('Drogleveransen')
	...
	while true do
		...
		local v = Config.CargoProviderLocation 
			if(Vdist(v.x, v.y, v.z, pos.x, pos.y, pos.z) < 2.0)then
				DisplayHelpText("Tryck ~INPUT_CONTEXT~ för att prata med ~b~Ricky")
			...
			if event_is_running then
				...
				if pVehicle == event_vehicle then
					...
					if delivery_point_distance < 50.0 then
					 ...
					 DrawMissionText("Sätt dig i fordonet!", 1000)
				end
			end
	end
end)
		...
		Citizen.Wait(5000)
				if event_is_running then
						...
						if IsPedDeadOrDying(GetPlayerPed(-1)) then
							ResetCargo()
							DisplayMissionFailed('Du dog!')
						end

						if GetVehicleEngineHealth(event_vehicle) < 20 and event_vehicle ~= nil then
							ResetCargo()
							DisplayMissionFailed('Leveransen blev skadad.')
						end

						if event_time_passed > 1800 then
							ResetCargo()
							DisplayMissionFailed('Leveransen blev för sen.')
						end
...
function DrawProviderBlip()
	...
	SetBlipColour(blip,1)
	BeginTextCommandSetBlipName("STRING")
	AddTextComponentString('Drogleverans')
	EndTextCommandSetBlipName(blip)
...
function CargoMenu()
	ClearMenu()
    options.menu_title = "Drogtransport"
    for i = 1, #Config.Scenarios do
    	Menu.addButton("Köp last - SEK " .. Config.Scenarios[i].CargoCost, "PurchaseCargo", i)
   	end
    Menu.addButton("Avbryt","CloseMenu",nil) 
end
...
function PurchaseCargo(scenario)
	...
	if event_is_running == true then
		...
		drawNotification("Du har redan startat en drogleverans.")
		goto done
	...
	ESX.TriggerServerCallback('esx_cargodelivery:getCopsOnline', function(police)
		...
		if police >= Config.Scenarios[scenario].MinCopsOnline then
			...
			if bought then
				...
				TriggerServerEvent('esx_addons_chrono:startCall', 'police', 'Larm: Ett obehörigt fordon har kört in i hamnen, regnummer: '  .. vehicle_plate,  {x = -767.64, y = -1448.52, z = 5.0}, dispatch)
				--TriggerServerEvent('esx_addons_chrono:startCall', 'police', 'Ett obehörigt fordon står på våran lastplats vid hamnen! Regnummer:  '  .. vehicle_plate .. ' , {x = -767.64, y = -1448.52, z = 5.0}, dispatch)
				--exports['mythic_notify']:DoCustomHudText('success', 'Vänta medans Hector hämtar fordonet åt dig..', 15000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
				...
				-- Tjöra bil höhöh
				driver = CreatePedInsideVehicle(event_vehicle, 4, Config.NPCHash, -1, 1, true)
				TaskVehicleDriveToCoordLongrange(driver, event_vehicle, 1012.79,-2911.6,5.9,20.0, 447, 10.0)

				Citizen.Wait(15000)
				if IsVehicleStopped(event_vehicle) then
					TaskLeaveAnyVehicle(driver, 1,1)
					--exports['mythic_notify']:DoCustomHudText('success', 'Sätt dig i fordonet..', 5000, { ['background-color'] = '#007ecc', ['color'] = '#fff' })
					Citizen.Wait(2000)
					SetPedCanPlayAmbientAnims(driver, true)
					RequestAnimDict("gestures@m@standing@casual")
					TaskPlayAnim(driver, "gestures@m@standing@casual" ,"gesture_come_here_soft" ,8.0, -8.0, -1, 0, 0, false, false, false )
					Citizen.Wait(1000)
					RequestAnimDict("mp_cp_stolen_tut")
					TaskPlayAnim(driver, "mp_cp_stolen_tut" ,"b_think" ,8.0, -8.0, -1, 0, 0, false, false, false )
					--exports['mythic_notify']:DoCustomHudText('success', 'Snuten är på väg, stick!', 3500, { ['background-color'] = '#ad0000', ['color'] = '#fff' })
					RequestAnimDict("mini@strip_club@idles@bouncer@base")
					TaskPlayAnim(driver, "mini@strip_club@idles@bouncer@base" ,"base" ,8.0, -8.0, -1, 0, 0, false, false, false )
					Citizen.Wait(5000)
					DeletePed(driver)
				end
				...
				SetBlipSprite(event_delivery_blip,479)
			...
			drawNotification("Det behövs minst ~b~" .. Config.Scenarios[scenario].MinCopsOnline .. " poliser ~w~i tjänst.")
