...
ESX.RegisterServerCallback('esx_cargodelivery:sellCargo', function(source, cb, price)
	...
	if Config.UsesBlackMoney then
		...
		xPlayer.addInventoryItem('cocaineleaf', price)
	...
	TriggerClientEvent('esx:showNotification', source, "Du fick ~r~" .. price .. "~w~ från lasten.")
	cb(true)
...
ESX.RegisterServerCallback('esx_cargodelivery:buyCargo', function(source, cb, price)
	...
	if (os.time() - LastDelivery) < 200.0 and LastDelivery ~= 0.0 then
		...
		TriggerClientEvent('esx:showNotification', source, "En transport är redan på gång.")
		cb(false)
		...
		if Config.UsesBlackMoney then
			...
			if xPlayer.getAccount('black_money').money >= price then
				...
				TriggerClientEvent('esx:showNotification', source, "Inte tillräckligt med ~r~pengar~w~.")
