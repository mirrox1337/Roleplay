...
function ClotheShopAdd(menu)
	...
	local tenues = NativeUI.CreateItem("Mina Kläder","")
	menu:AddItem(tenues)
	local haut = _menuPool:AddSubMenu(menu, "Topp","",true,true)
	local bras = _menuPool:AddSubMenu(menu, "Armar","",true,true)
	local tsh = _menuPool:AddSubMenu(menu, "T-shirt","",true,true)
	local bas = _menuPool:AddSubMenu(menu, "Byxor","",true,true)
	...
	menu.OnItemSelect = function(_,_,ind)
		if ind == 1 then
			...
			local clotheShop2 = NativeUI.CreateMenu("", "Butik", 5, 100,"shopui_title_midfashion","shopui_title_midfashion")
...
function SavedTenues(menu)
	p = NativeUI.CreateItem("Spara den här outfiten","")
	menu:AddItem(p)
	local sqk = nil
	...
	ESX.TriggerServerCallback('parow:GetTenues', function(skin)
		if #skin == 0 then
			menu:AddItem(NativeUI.CreateItem("Tomt",""))
		end
		for i = 1, #skin,1 do
...
function montreMenu(menu)
	...
	for i = -1,19,1 do
		...
		v = NativeUI.CreateListItem("Klocka #"..ind, amount, 1, "",5)
...
function chainFct(menu)
	...
	for i = -1,GetNumberOfPedDrawableVariations(playerPed,7),1 do
		...
		v = NativeUI.CreateListItem("Halsband #"..ind, amount, 1, "",5)
...
function tshirtmenu(menu)
	...
	for i = 0,GetNumberOfPedDrawableVariations(playerPed,8)-1,1 do
		--		Citizen.Wait(2)
				...
				x = NativeUI.CreateListItem("T-Shirt #"..i, amount, 1, "",5)
...
function sacFct(menu)
	playerPed = GetPlayerPed(-1)
	n = NativeUI.CreateItem("Väska","")
	menu:AddItem(n)
	c = NativeUI.CreateItem("Svart Väska","")
	menu:AddItem(c)
	cx = NativeUI.CreateItem("Ingen väska","")
	menu:AddItem(cx)
...
local clotheShop = NativeUI.CreateMenu("", "Butik", 5, 100,"shopui_title_midfashion","shopui_title_midfashion")
...
AddEventHandler('clothesShop:hasEnteredMarker', function(zone)
	...
	CurrentAction6Msg  = 'Tryck ~INPUT_CONTEXT~ för öppna ~g~Klädbutiken'
...
function OpenAccessMenus()
    _menuPool:CloseAllMenus()
    mainMenu = NativeUI.CreateMenu("Mina Accessoarer", "Tillgängliga accessoarer", 5, 200)
