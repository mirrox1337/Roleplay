...
function ClotheShopAdd(menu)
	...
	--local chapeau = _menuPool:AddSubMenu(menu, "Huvudbonad","",true,true)
	--local gil = _menuPool:AddSubMenu(menu, "Skottsäker Vest","",true,true)
	local sac = _menuPool:AddSubMenu(menu, "Ryggsäck","",true,true)
	--local montre = _menuPool:AddSubMenu(menu, "Klocka","",true,true)
	local chain = _menuPool:AddSubMenu(menu, "Kedja","",true,true)
	...
	menu.OnItemSelect = function(_,_,ind)
		if ind == 1 then
			...
			local clotheShop2 = NativeUI.CreateMenu("", "Klädbutik", 5, 100,"shopui_title_midfashion","shopui_title_midfashion")
...
function SavedTenues(menu)
	p = NativeUI.CreateItem("Spara outfit","")
	menu:AddItem(p)
	local sqk = nil
	...
	ESX.TriggerServerCallback('parow:GetTenues', function(skin)
		...
		for i = 1, #skin,1 do
			...
			p = NativeUI.CreateItem("Sätt på","")
			k = NativeUI.CreateItem("Renommer","")
			l = NativeUI.CreateItem("Ta Bort","")
			m:AddItem(p)
			m:AddItem(k)
			m:AddItem(l)
...
function tshirtmenu(menu)
	...
	for i = 0,GetNumberOfPedDrawableVariations(playerPed,8)-1,1 do
		--		Citizen.Wait(2)
				...
				x = NativeUI.CreateListItem("Topp #"..i, amount, 1, "",5)
