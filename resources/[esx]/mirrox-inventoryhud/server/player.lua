...
ESX.RegisterServerCallback('mirrox-inventoryhud:getPlayerInventory', function(source, cb)
...
function ensurePlayerInventory(player)
    getInventory(player.identifier, 'player', function(result)
        ...
        for _, esxItem in pairs(player.getInventory()) do
            print('Lägger till ' .. esxItem.name .. ' ' .. esxItem.count .. ' ' .. esxItem.weight)
...
RegisterServerEvent('mirrox-inventoryhud:notifyImpendingRemoval')
AddEventHandler('mirrox-inventoryhud:notifyImpendingRemoval', function(item, count, playerSource)
...
RegisterServerEvent('mirrox-inventoryhud:notifyImpendingAddition')
AddEventHandler('mirrox-inventoryhud:notifyImpendingAddition', function(item, count, playerSource)
    local _source = playerSource or source
...
AddEventHandler('esx:onRemoveInventoryItem', function(source, item, count)
    ...
    TriggerClientEvent('mirrox-inventoryhud:showItemUse', source, {
        { id = item.name, label = item.label, qty = count, msg = 'Kastar' }
    })
...
AddEventHandler('esx:onAddInventoryItem', function(source, esxItem, count)
    local player = ESX.GetPlayerFromId(source)
    TriggerClientEvent('mirrox-inventoryhud:showItemUse', source, {
        { id = esxItem.name, label = esxItem.label, qty = count, msg = 'Plockar upp' }
    })
    applyToInventory(player.identifier, 'player', function(inventory)
