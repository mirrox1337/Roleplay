...
function closeInventory() {
    InventoryLog('Stänger inventoryt');
    EndDragging();
    $.post("http://mirrox-inventoryhud/NUIFocusOff", JSON.stringify({}));
}
...
$(document).ready(function () {
    ...
    $('#use').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.usable) {
                InventoryLog('Använde ' + ' och stängde ' + itemData.closeUi);
                $.post("http://mirrox-inventoryhud/UseItem", JSON.stringify({
    ...
    $('#give').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                $.post("http://mirrox-inventoryhud/GetNearPlayers", JSON.stringify({
                    item: itemData,
                    action: 'give'
                }));
    ...
    $("#pay").mouseenter(function () {
       ...
       $("#seize").mouseenter(function () {
        ...
        InventoryLog('Konfiskerar pengar från  ' + destinationOwner);
        $.post("http://mirrox-inventoryhud/SeizeCash", JSON.stringify({
    ...
    $("#steal").mouseenter(function () {
        ...
        $.post("http://mirrox-inventoryhud/StealCash", JSON.stringify({
            target: destinationOwner
        }));
    });
    ...
    $('#drop').click(function (event, ui) {
        if (dragging) {
            ...
            if (itemData.canRemove) {
                InventoryLog('Kastar ' + dropCount + ' ' + itemData.label + ' på marken');
                $.post("http://mirrox-inventoryhud/DropItem", JSON.stringify({
                    item: itemData,
                    qty: dropCount
                }));
                successAudio.play();
        ...
        if (itemData !== undefined) {
            ...
            if (!itemData.unique) {
                if (itemData.stackable) {
                    ...
                    $('.tooltip-div').find('.tooltip-uniqueness').html("Inte Unik - Går inte stacka");
                }
            } else {
            ...
            if (itemData.description !== undefined) {
                ...
                $('.tooltip-div').find('.tooltip-desc').html("Information Saknas");
            }
            ...
            if (itemData.staticMeta !== undefined || itemData.staticMeta !== "") {
                if (itemData.type === 1) {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Registrerad Användare</div> : <div class="meta-val">' + itemData.staticMeta.owner + '</div></div>');
                } else if (itemData.itemId === 'license') {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Namn</div> : <div class="meta-val">' + itemData.staticMeta.name + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Utfärdat</div> : <div class="meta-val">' + itemData.staticMeta.issuedDate + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Längd</div> : <div class="meta-val">' + itemData.staticMeta.height + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Födelsedaturm</div> : <div class="meta-val">' + itemData.staticMeta.dob + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Telefon Nummer</div> : <div class="meta-val">' + itemData.staticMeta.phone + '</div></div>');
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Medborgar ID</div> : <div class="meta-val">' + itemData.staticMeta.id + '-' + itemData.staticMeta.user + '</div></div>');

                    if (itemData.staticMeta.endorsements !== undefined) {
                        $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key">Signatur</div> : <div class="meta-val">' + itemData.staticMeta.endorsements + '</div></div>');
                    }
                } else if (itemData.itemId === 'gold') {
                    $('.tooltip-div').find('.tooltip-meta').append('<div class="meta-entry"><div class="meta-key"></div> : <div class="meta-val">Serienummer graverat och registrerad till Never Ending</div></div>');
                }
            } else {
                $('.tooltip-div').find('.tooltip-meta').html("Information Saknas");
            }
            $('.tooltip-div').show();
        }
    });
...
function AttemptDropInEmptySlot(origin, destination, moveQty) {
    ...
    if (result === -1) {
        ...
        if (moveQty === item.qty) {
            ...
            $.post("http://mirrox-inventoryhud/MoveToEmpty", JSON.stringify({
            ...
            $.post("http://mirrox-inventoryhud/EmptySplitStack", JSON.stringify({
                originOwner: origin.parent().data('invOwner'),
                originSlot: origin.data('slot'),
                originTier: origin.parent().data('invTier'),
        ...
        if (result === 1) {
            ...
            InventoryLog("Ägare var inte definierad");
        }
    }
}
    ...
    if (result === -1) {
        ...
        if (originItem.itemId === destinationItem.itemId && destinationItem.stackable) {
            ...
            if (moveQty != originItem.qty && destinationItem.qty + moveQty <= destinationItem.max) {
                ...
                $.post("http://mirrox-inventoryhud/SplitStack", JSON.stringify({
                    originOwner: origin.parent().data('invOwner'),
                    originTier: origin.parent().data('invTier'),
                ...
                if (destinationItem.qty === destinationItem.max) {
                    ...
                    InventoryLog('Swappar ' + originItem.label + ' i  ' + destination.parent().data('invOwner') + ' Slot ' + originItem.slot + ' med ' + destinationItem.label + ' In ' + origin.data('invOwner') + ' Slot ' + destinationItem.slot);
                    $.post("http://mirrox-inventoryhud/SwapItems", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        destinationOwner: destination.parent().data('invOwner'),
                        destinationItem: destination.find('.item').data('item'),
                    ...
                    successAudio.play();
                    InventoryLog('Slår ihop ' + originItem.label + ' i ' + origin.data('invOwner') + ' Slot ' + originItem.slot + ' till ' + destination.parent().data('invOwner') + ' Slot' + destinationItem.slot);
                    $.post("http://mirrox-inventoryhud/CombineStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originSlot: origin.data('slot'),
                        originTier: origin.parent().data('invTier'),
                        originItem: originItem,
                    ...
                    InventoryLog('Adderar ' + originItem.label + ' till existerande stack ' + destination.parent().data('invOwner') + ' Slot ' + destinationItem.slot);
                    $.post("http://mirrox-inventoryhud/TopoffStack", JSON.stringify({
                        originOwner: origin.parent().data('invOwner'),
                        originItem: origin.find('.item').data('item'),
                        originSlot: originItem.slot,
                        originTier: origin.parent().data('invTier'),
...
$('.popup-body').on('click', '.player', function () {
    ...
    if (action === "nearPlayersGive") {
        ...
        if (givingItem != null) {
            ...
            InventoryLog(`Ger ${count} ${givingItem.label} till spelare med ServerID ${target}`);
            $.post("http://mirrox-inventoryhud/GiveItem", JSON.stringify({
                target: target,
                item: givingItem,
                count: count
            }), function (status) {
        ...
        InventoryLog(`Ger ${count} kontanter till spelare med Server ID ${target}`);
        $.post("http://mirrox-inventoryhud/GiveCash", JSON.stringify({
            target: target,
            item: 'cash',
            count: count
        }), function (status) {
...
function ItemUsed(alerts) {
    ...
    $('#use-alert').hide('slide', {direction: 'left'}, 500, function () {
        ...
        $.each(alerts, function (index, data) {
            $('#use-alert').append(`<div class="slot alert-${index}""><div class="item"><div class="item-count">${data.qty}</div></div><div class="alert-text">${data.message}<div class="item-name">${data.item.label}</div></div>`)
                .ready(function () {
    ...
    $('#use-alert').show('slide', { direction: 'up' }, 500, function() {
    ...
    if ($('#action-bar').is(':visible')) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (items[i] != null) {
                ...
                $(`.slot-${i}`).find('.item-name').html('TOM');
        ...
        for (let i = 0; i < 5; i++) {
            if (items[i] != null) {
                ...
                $('#action-bar').append(`<div class="slot slot-${i}" data-empty="true"><div class="item"><div class="item-count"></div><div class="item-name">TOM</div><div class="item-keybind">${i + 1}</div></div></div>`);
                $(`.slot-${i}`).find('.item').css('background-image', 'none');
            }
        }
