...
RegisterNetEvent('mirrox-base:registerMarker')
AddEventHandler('mirrox-base:registerMarker', function(marker)
    ...
    if marker.command then
        RegisterCommand(marker.command.key, function(src, args, raw)
            ...
            if raw == command then
                TriggerEvent('mirrox-base:triggerCurrentMarkerAction')
            end
        end)
    end
...
RegisterNetEvent('mirrox-base:removeMarker')
AddEventHandler('mirrox-base:removeMarker', function(name)
    markers[name] = nil
end)
...
Citizen.CreateThread(function()
    ...
    while true do
            ...
            if distance < Config.DrawDistance and v.shouldDraw() then
                ...
                elseif v.type ~= -1 then
                    ...
                    elseif distance < Config.DrawDistance then
                        ESX.Game.Utils.DrawText3D(v.coords, "~p~" ..v.name, 0.6)
                    end
            end
        end
    end
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerEvent('mirrox-base:hasEnteredMarker', lastMarker)
        end
        if not hasExited and not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('mirrox-base:hasExitedMarker')
        end
    end

end)

AddEventHandler('mirrox-base:hasExitedMarker', function()
    CurrentMarker = nil
    ESX.UI.Menu.CloseAll()
end)

AddEventHandler('mirrox-base:hasEnteredMarker', function(marker)
...
RegisterNetEvent('mirrox-base:triggerCurrentMarkerAction')
AddEventHandler('mirrox-base:triggerCurrentMarkerAction', function()
    if CurrentMarker and CurrentMarker.action ~= nil then
        CurrentMarker.action(CurrentMarker)
    end
end)

RegisterNetEvent('mirrox-base:registerBlip')
AddEventHandler('mirrox-base:registerBlip', function(blip)
...
RegisterNetEvent('mirrox-base:updateBlip')
AddEventHandler('mirrox-base:updateBlip', function(blip, debug)
    ...
    if blip.coords then
        ...
        if _blip and GetBlipCoords(_blip) ~= blip.coords then
            ...
            TriggerEvent('mirrox-base:registerBlip', tempBlip)
            return
        end
