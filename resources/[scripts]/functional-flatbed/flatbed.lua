...
local bedController = {vector3(-2.5, -3.8, 0.0), vector3(0.0, 0.0, 0.0)}
...
AddTextEntry(controllerMessageLoweredCar, "~INPUT_CONTEXT~ för att dra ~y~upp ~w~släp.~n~ ~INPUT_DETONATE~ för att ~r~lossa ~w~fordon.")
AddTextEntry(controllerMessageLoweredNoCar, "~INPUT_CONTEXT~ för att dra ~y~upp ~w~släp.~n~ ~INPUT_DETONATE~ för att ~g~binda ~w~fordon.")
AddTextEntry(controllerMessageRaised, "~INPUT_CONTEXT~ för att sänka ~y~ner ~w~släp.")
...
function drawMarker(pos)
    ...
    if dist < 25.0 then
        DrawMarker(2, pos, vector3(0.0, 0.0, 0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 0.5, 1.0), 255, 237, 0, 150, 0, 1, 0, 0, 0)
        if dist < 1.5 then
            return true
        end
    end
