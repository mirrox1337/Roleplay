...
AddEventHandler('esx_duty:hasEnteredMarker', function (zone)
  ...
  if zone == 'BennysDuty' then
    CurrentAction     = 'bennys_duty'
    CurrentActionMsg  = _U('duty')
    CurrentActionData = {}
  end
  if zone == 'BennysDuty' then
      ...
      if IsControlPressed(0, Keys['E']) then
        ...
        if CurrentAction == 'cardealer_duty' then
          if PlayerData.job.name == 'cardealer' or PlayerData.job.name == 'offcardealer' then
            TriggerServerEvent('duty:cardealer')
          ...
          --sendNotification(_U('notpol'), 'error', 5000)
          exports['mythic_notify']:SendAlert('error', _U('notcar'))
          Wait(1000)
          end
        end

        if CurrentAction == 'bennys_duty' then
          if PlayerData.job.name == 'bennys' or PlayerData.job.name == 'offbennys' then
            TriggerServerEvent('duty:bennys')
          ...
          --sendNotification(_U('notpol'), 'error', 5000)
          exports['mythic_notify']:SendAlert('error', _U('notben'))
          Wait(1000)
          end
        end
        ...
        if CurrentAction == 'taxi_duty' then
          ...
          --sendNotification(_U('notpol'), 'error', 5000)
          exports['mythic_notify']:SendAlert('error', _U('nottax'))
          Wait(1000)
          end
        end
        ...
        if CurrentAction == 'mechanic_duty' then
          ...
          --sendNotification(_U('notsec'), 'error', 5000)
  ...
  while true do
    ...
    for k,v in pairs(Config.Zones) do
      if(v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance) then
        DrawMarker(6, v.Pos.x, v.Pos.y, v.Pos.z, 0, 0, 0, -1, 0, 0, v.Size.x, v.Size.y, v.Size.z, 0, 119, 119, 50, 0, 0, 0, 0)
